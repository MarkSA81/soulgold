...
#if MAX_MON_ITEMS > 1
AI_SINGLE_BATTLE_TEST("AI: Belch has nonzero score after eating a berry (Items)")
{
    GIVEN {
        AI_FLAGS(AI_FLAG_CHECK_BAD_MOVE | AI_FLAG_CHECK_VIABILITY | AI_FLAG_TRY_TO_FAINT);
        PLAYER(SPECIES_BAYLEEF) { Level(18); Moves(MOVE_MUD_SHOT, MOVE_SCRATCH); }
...
SINGLE_BATTLE_TEST("Belch cannot be used if the user has not eaten a berry (Items)")
{
    enum Item item = ITEM_NONE;
    PARAMETRIZE { item = ITEM_NONE; }
    PARAMETRIZE { item = ITEM_ORAN_BERRY; }
...
SINGLE_BATTLE_TEST("Belch can still be used after switching out (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_STUFF_CHEEKS) == EFFECT_STUFF_CHEEKS);
...
SINGLE_BATTLE_TEST("Belch can still be used after fainting (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_STUFF_CHEEKS) == EFFECT_STUFF_CHEEKS);
        ASSUME(GetMoveFixedHPDamage(MOVE_DRAGON_RAGE) == 40);
...
SINGLE_BATTLE_TEST("Belch can still be used after restoring the consumed berry (Items)")
{
    GIVEN {
        ASSUME(GetMoveEffect(MOVE_STUFF_CHEEKS) == EFFECT_STUFF_CHEEKS);
        ASSUME(GetMoveEffect(MOVE_RECYCLE) == EFFECT_RECYCLE);
